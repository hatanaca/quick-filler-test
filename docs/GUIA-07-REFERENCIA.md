...
| `packages/infrastructure/src/web/routes/auth.route.ts`                               | Rotas de autenticação (login, refresh, logout, me)                  | Sistema de autenticação JWT                            |
| `packages/infrastructure/src/web/middleware/auth.ts`                                 | Verificação JWT, geração de tokens                                  | Middleware de autenticação                             |
| `packages/infrastructure/src/ocr/image-preprocessor.ts`                              | Grayscale, Sauvola binarization, deskew, detecção de tinta vermelha | Pré-processamento de imagem para OCR                   |
