...
describe('Transcription', () => {
  it('cria com status PROCESSANDO', () => {
    const t = Transcription.create({ id: TranscriptionId.from('abc'), tipo })
  ...
  it('updateValue substitui value quando concluído', () => {
    const t = Transcription.create({ id: TranscriptionId.from('abc'), tipo })
    const original = { pages: [PageCartaoPonto.from({ page: 1, days: [DayRecord.from({ date_raw: '21/05/2019', punches: [] })] })] }
    t.complete(original)
    const corrigido = { pages: [PageCartaoPonto.from({ page: 1, days: [DayRecord.from({ date_raw: '22/05/2019', punches: [] })] })] }
